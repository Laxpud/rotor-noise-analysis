...
for OBS_Number in OBS_range:
    ...
    ax.legend(
        ncol=3,                        # 保持3列布局
        loc='lower center',             # 图例自身的锚点：右下角
        # bbox_to_anchor=(1.0, -0.21),     # 锚定到坐标轴的(1,0)位置（x轴最右、y轴最下）
        # transform=ax.transAxes,        # 使用坐标轴相对坐标系（0-1范围）
        # borderaxespad=0.1,             # 图例与锚点的间距（可微调，0为紧贴）
