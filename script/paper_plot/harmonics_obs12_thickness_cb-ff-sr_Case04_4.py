...
ymin, ymax, ystep = -50, 100, 25
ax.set_ylim([ymin, ymax])
ax.set_yticks(np.arange(ymin, ymax + 1, ystep))
# ----------- 散点图
# 数据
ax.scatter(data_1['Harmonic Order'], data_1['SPL_FF_Thickness(dB)'], label='Free-field', color='grey', marker='o', alpha=0.5, zorder=3, linewidths=scatter_lw)
ax.scatter(data_1['Harmonic Order'], data_1['SPL_SR_Thickness(dB)'], label='Reflected', color=colors[0], marker='s', alpha=0.8, zorder=2, linewidths=scatter_lw)
ax.scatter(data_1['Harmonic Order'], data_1['SPL_merged_Thickness(dB)'], label='Combined', color=colors_wong[1], marker='^', alpha=0.8, zorder=2, linewidths=scatter_lw)
